...
-export([member_status/1, ring_status/1, print_member_status/2,
         ...
         clear_staged/1, transfer_limit/1,transfers/1]).
...
transfers([PeriodStr,MaxStr]) ->
    Period = try list_to_integer(PeriodStr)
             catch _:_ -> undefined
             end,
    Max = try list_to_integer(MaxStr)
          catch _:_ -> undefined
          end,
    periodically(fun() -> transfers([]) end, Period, Max);
transfers([P]) ->
    transfers([P, undefined]);
transfers([]) ->
    try
...
periodically(F, undefined, _) ->
    F();
periodically(_, _, Max) when Max =< 0->
    ok;
periodically(F, WaitSeconds, Max) ->
    NextMax = case Max of
                  undefined -> undefined;
                  _ -> Max - 1
              end,
    ...
    periodically(F, WaitSeconds, NextMax).
